  ...
  return withSessionLock(cwd, input.session_id, async () => {
    ...
    if (
      ...
      return buildFeedbackResponse({
        ...
        result: reviewResultFromFeedback(
...
function reviewResultFromFeedback(
  round: number,
  feedback: ReviewRoundResult["feedback"],
): ReviewRoundResult {
...
function buildFeedbackResponse(args: {
  session: NonNullable<ReturnType<typeof readSessionState>>;
  cwd: string;
  config: PlanpongConfig;
  result: ReviewRoundResult;
  idempotentReplay: boolean;
  resumedIncompleteRound?: boolean;
}) {
