...
const inputSchema = {
  ...
  expected_round: z
    ...
    .positive()
    .describe(
      "The feedback round this revision responds to. Must equal session.currentRound.",
    ),
  cwd: z
    .string()
    .optional()
...
export async function reviseHandler(input: {
  ...
  return withSessionLock(cwd, input.session_id, async () => {
    ...
    if (!feedback) {
      return errorResponse(
        `No feedback found for session ${session.id} round ${input.expected_round}. Call planpong_get_feedback first.`,
      );
    }
    const existingResponse = readRoundResponse(
      cwd,
      session.id,
      input.expected_round,
    );
    if (existingResponse) {
    ...
    if (!plannerProvider) {
      return errorResponse(
        `Planner provider not found: ${session.planner.provider}`,
      );
    }
...
function tallyResponses(responses: RevisionRoundResult["revision"]["responses"]) {
  let accepted = 0;
  let rejected = 0;
  let deferred = 0;
...
function buildRevisionResponse(args: RevisionRoundResult & {
  statusLine: string;
  feedback: NonNullable<ReturnType<typeof readRoundFeedback>>;
  idempotentReplay: boolean;
}) {
  const unverifiedRejected = args.revision.responses.filter(
    (r) =>
      r.action === "rejected" &&
