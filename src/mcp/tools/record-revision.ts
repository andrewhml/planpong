...
export async function recordRevisionHandler(input: {
  ...
  if (input.expected_round !== session.currentRound) {
    return errorResponse(
      `round ${input.expected_round} already finalized (current round is ${session.currentRound})`,
      {
  ...
  const unverifiedRejected = input.responses.filter(
    (r) =>
      r.action === "rejected" &&
  ...
  return {
    content: [
      { type: "text" as const, text: statusLine },
