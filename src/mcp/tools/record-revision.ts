...
const inputSchema = {
  ...
  expected_round: z
    ...
    .nonnegative()
    .describe(
      "The round this revision responds to. Must equal session.currentRound. Catches double-submission and stale tool calls.",
    ),
  responses: z
    .array(IssueResponseSchema)
    .describe(
      "One response per issue from the round's feedback. Every issue.id MUST appear here.",
    ),
  cwd: z
    .string()
    .optional()
  ...
  return withSessionLock(cwd, input.session_id, async () => {
    ...
    if (session.status !== "in_review") {
      return errorResponse(
        `Session status is '${session.status}', expected 'in_review'`,
      );
    }

    if (session.plannerMode !== "inline") {
      return errorResponse(
        `session is in ${session.plannerMode} planner mode — use planpong_revise instead`,
    ...
    if (input.expected_round !== session.currentRound) {
      const relation =
        input.expected_round < session.currentRound ? "stale" : "out-of-order";
      const message =
        relation === "stale"
    ...
    if (!feedback) {
      return errorResponse(
        `No feedback found for session ${session.id} round ${session.currentRound}. Call planpong_get_feedback first.`,
      );
    }

    const existingResponse = readRoundResponse(
      cwd,
      session.id,
    ...
    if (
      ...
      const accepted = existingResponse.responses.filter(
        (r) => r.action === "accepted",
      ).length;
      const rejected = existingResponse.responses.filter(
        (r) => r.action === "rejected",
      ).length;
    ...
    if (existingResponse) {
      return errorResponse(
        `round ${session.currentRound} already finalized with different responses`,
        {
          current_round: session.currentRound,
    ...
    if (missing.length > 0) {
      return errorResponse(
        `responses missing for issue(s): ${missing.join(", ")}. Provide one response per feedback issue.`,
...
function countUnverifiedRejected(responses: IssueResponse[]): number {
...
function buildRecordRevisionResponse(args: {
  round: number;
  feedback: NonNullable<ReturnType<typeof readRoundFeedback>>;
  revision: PlannerRevision;
  responses: IssueResponse[];
  accepted: number;
  rejected: number;
  deferred: number;
  unverifiedRejected: number;
  planUpdated: boolean;
  statusLine: string;
  ...
  return {
    content: [
      { type: "text" as const, text: args.statusLine },
...
export function registerRecordRevision(server: McpServer): void {
  server.tool(
    "planpong_record_revision",
    "Inline-mode revision: log the agent's per-issue responses and advance session bookkeeping. The agent must have already edited the plan with its own Edit/Write tools. Use only when planpong_start_review was called with planner_mode: 'inline'. Otherwise call planpong_revise.",
    inputSchema,
    recordRevisionHandler,
  );
}
