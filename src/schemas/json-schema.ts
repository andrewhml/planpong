...
function stripObservabilityFields(node: unknown): unknown {
  if (Array.isArray(node)) return node.map(stripObservabilityFields);
    ...
    for (const [key, value] of Object.entries(obj)) {
      if (key === "properties" && value && typeof value === "object") {
        ...
        result[key] = value.filter(
          (k) => typeof k !== "string" || !OBSERVABILITY_FIELDS.has(k),
        );
      } else {
 ...
 */
function toOpenAIStrict(node: unknown): unknown {
 ...
 */
export function getFeedbackJsonSchemaForPhase(
  phase: ReviewPhase,
