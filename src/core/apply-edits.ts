...
function normalizeTrailingWhitespace(s: string): string {
  return s.replace(/\r\n/g, "\n").replace(/[ \t]+\n/g, "\n");
}

function parseSections(plan: string): SectionBoundary[] {
  ...
  const headingLines: Array<{ index: number; level: number; heading: string }> = [];
...
function findSection(
  ...
  label: string,
): { boundary: SectionBoundary; duplicateCount: number } | null {
...
function indexOfAllOccurrences(haystack: string, needle: string): number[] {
  const offsets: number[] = [];
  if (needle.length === 0) return offsets;
  let from = 0;
  while (true) {
    const idx = haystack.indexOf(needle, from);
    if (idx === -1) return offsets;
    offsets.push(idx);
    from = idx + 1;
  }
}

function modifiesStatusLine(plan: string, range: { start: number; end: number }): boolean {
 ...
 */
export function applyEdits(plan: string, edits: ReplaceEdit[]): ApplyEditsResult {
  ...
  for (const edit of edits) {
    ...
    if (offsets.length === 0) {
      ...
      const planWide = indexOfAllOccurrences(
        normalizeTrailingWhitespace(working),
        normalizedBefore,
      );
    ...
    if (unNormalizedOffsets.length === 1) {
      ...
      const restored = candidate.match(
        new RegExp(
          edit.before
            .replace(/\r\n/g, "\n")
            .replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
 ...
 */
export function summarizeApply(result: ApplyEditsResult): string {
 ...
 */
export function logFailures(prefix: string, failures: EditFailure[]): void {
  for (const f of failures) {
    const detail = f.diagnostic ? ` (${f.diagnostic})` : "";
    process.stderr.write(
      `[planpong] ${prefix}: edit failed (${f.reason}) section="${f.section_searched ?? "?"}"${detail}\n`,
    );
  }
}
