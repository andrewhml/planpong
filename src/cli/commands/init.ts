...
import type { ModelCatalog, Provider } from "../../providers/types.js";
 ...
 * written to disk.
 */
export const CLI_DEFAULT: unique symbol = Symbol("cli-default");
...
export function buildModelChoices(
...
): { choices: Choice[]; default: WizardChoice } {
  ...
  if (providerChanged || !diskModel) return { choices, default: CLI_DEFAULT };
 ...
 */
export function buildEffortChoices(
  catalog: ModelCatalog,
  model: WizardChoice,
  providerChanged: boolean,
  diskEffort: string | undefined,
): { choices: Choice[]; default: WizardChoice; hint?: string } | null {
  if (catalog.allEfforts.length === 0) return null;
  const known = typeof model === "string" ? catalog.models.find((m) => m.id === model) : undefined;
  const available =
    model === CLI_DEFAULT ? catalog.efforts : known ? known.efforts : catalog.allEfforts;
  ...
  const hint =
    model === CLI_DEFAULT && catalog.efforts.length < catalog.allEfforts.length
 ...
 */
export function readDiskSnapshot(cwd: string): {
  path: string | null;
  snapshot: DiskSnapshot;
} {
  const path = findConfigPath(cwd);
  if (!path) return { path: null, snapshot: {} };
  const raw = readFileSync(path, "utf-8");
  return { path, snapshot: (parseYaml(raw) as DiskSnapshot) ?? {} };
}
...
export function answersToPicks(
  ...
  const add = (key: string, answer: unknown, diskValue: unknown): void => {
    if (answer === CLI_DEFAULT) {
      ...
      if (diskValue !== undefined) picks.push({ key, unset: true });
      return;
    }
...
async function runWizard(cwd: string): Promise<void> {
  ...
  const askRole = async (
    ...
    label: string,
    providerName: string,
  ): Promise<{ model: WizardChoice; effort: WizardChoice }> => {
    const provider = statuses.find((s) => s.provider.name === providerName)?.provider;
    if (!provider) return { model: CLI_DEFAULT, effort: CLI_DEFAULT };
    ...
    const providerChanged =
      providerName !== (diskRole?.provider ?? DEFAULT_CONFIG[role].provider);

    const modelChoices = buildModelChoices(providerName, catalog, providerChanged, diskRole?.model);
    const model = await select<WizardChoice>({
      message: `${label} model:`,
    ...
    if (!effortChoices) return { model, effort: CLI_DEFAULT };
    if (effortChoices.hint) console.log(chalk.dim(`  ${effortChoices.hint}`));
    const effort = await select<WizardChoice>({
