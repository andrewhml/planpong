 ...
 */
export function buildPlanDiff(prev, next) {
    if (prev === next)
        return "(no changes)";
    const a = prev.split("\n");
    const b = next.split("\n");
        ...
        if (op.kind === "eq") {
            if (current && k - lastChangeIdx <= CONTEXT_LINES) {
                ...
                if (last && last.type === "context")
                    ...
                    current.push({ type: "context", lines: [op.line] });
            }
        }
        else {
            ...
            if (!current) {
                ...
                const ctx = [];
                for (let p = k - 1; p >= 0 && ctx.length < CONTEXT_LINES && ops[p].kind === "eq"; p--) {
                    ctx.unshift(ops[p].line);
                }
                if (ctx.length)
                    current.push({ type: "context", lines: ctx });
            }
    ...
    const rendered = hunks
        .map((blocks) => blocks
        .map((block) => {
        const prefix = block.type === "context" ? "  " : block.type === "removed" ? "- " : "+ ";
